...
predicate acyclick(Node root) = 
  if root == null then true else acc(root.val) * acc(root.next) * acyclic(root.next)
```
> Where `*` is the separating conjunction, which preds must refer tod different heap locations, and `acc(root.val)` is the accessibility predicate, which denotes premission to access a heap location.
